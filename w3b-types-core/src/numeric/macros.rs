...
macro_rules! impl_num {
    ...
    (@common $num:ident, $n_bytes:literal) => {
        ...
        impl $num {
            ...
            #[inline]
            pub fn from_hex_unprefixed(hex: impl AsRef<str>) -> Result<Self, $crate::hex::HexError> {
                let mut repr = [0; Self::NUM_BYTES];
                $crate::hex::unprefixed::write_left_expanded_into(hex.as_ref(), &mut repr)?;
                Ok(Self(repr))
            }
            ...
            pub fn to_hex_unprefixed(&self) -> String {
                $crate::hex::unprefixed::read(self.as_bytes())
            }
        }
